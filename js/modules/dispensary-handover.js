...
const HANDOVER_STORAGE_KEY = 'amcal_dispensary_handovers';

/* ==========================================================================
   PART 1: DIGITAL DISPENSARY CLINICAL HANDOVER ENGINE
   ========================================================================== */
...
function openDispensaryHandoverModal() {
  ...
  // Pre-fill outgoing pharmacist with logged-in user
  ...
  if (outgoingInp && (!outgoingInp.value || outgoingInp.value.trim() === '')) {
    outgoingInp.value = state.currentUser?.name || 'Peter Kim';
  }
...
async function handleSaveHandover(event) {
  ...
  const fridgeTempRaw = document.getElementById('handover-fridge-temp')?.value;
...
function renderAllHandsDeckSlide() {
  const slide = SLIDE_DATA[deckCurrentSlide];
  if (!slide) return;
  ...
  if (dotsContainer) {
    dotsContainer.innerHTML = Array.from({ length: TOTAL_SLIDES }).map((_, i) => `
      <button type="button" class="deck-dot ${i === deckCurrentSlide ? 'active' : ''}" onclick="jumpToAllHandsSlide(${i})" title="Slide ${i + 1}" style="width: ${i === deckCurrentSlide ? '22px' : '8px'}; height: 8px; border-radius: 4px; border: none; background: ${i === deckCurrentSlide ? 'var(--accent-cyan)' : 'rgba(255,255,255,0.25)'}; transition: all 0.2s ease; cursor: pointer; padding: 0;"></button>
    `).join('');
  }
}
